...
use Modules\Core\Foundation\Asset\Manager\HappyMobilesAssetManager;
use Modules\Core\Foundation\Asset\Pipeline\HappyMobilesAssetPipeline;
...
class AssetServiceProvider extends ServiceProvider
{
    ...
    {
        $this->app->singleton(AssetManager::class, HappyMobilesAssetManager::class);

        $this->app->singleton(AssetPipeline::class, function ($app) {
            return new HappyMobilesAssetPipeline($app[AssetManager::class]);
        });
    }
    ...
    private function addThemesAssets()
    {
        ...
        if (! is_null($assets = config("happymobiles.themes.{$theme}.assets"))) {
    ...
    private function addModulesAssets()
    {
        foreach ($this->app['modules']->allEnabled() as $module) {
            $assets = config("happymobiles.modules.{$module->getAlias()}.assets");
