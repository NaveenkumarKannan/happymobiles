...
class CurrencyExchangeRateServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $this->app->singleton(CurrencyRateExchanger::class, function () {
            ...
            $options = config("happymobiles.modules.currency.config.services.{$service}");
    ...
    {
        config([
            'happymobiles.modules.currency.config.services.fixer.access_key' => setting('fixer_access_key'),
            'happymobiles.modules.currency.config.services.forge.api_key' => setting('forge_api_key'),
            'happymobiles.modules.currency.config.services.currency_data_feed.api_key' => setting('currency_data_feed_api_key'),
        ]);
    }
}
